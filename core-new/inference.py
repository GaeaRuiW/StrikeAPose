...
def main(input_vedio_file,out_vedio_file,out_json_file):
    ...
    while True:
        ...
        if not ret:
            ...
            cv2.line(frame, key_points[0], key_points[1], (0, 255, 255), thickness=2, lineType=cv2.LINE_8, shift=0)
            cv2.line(frame, key_points[1], key_points[3], (0, 255, 255), thickness=2, lineType=cv2.LINE_8, shift=0)
            cv2.line(frame,  key_points[3],  key_points[2], (0, 255, 255), thickness=2, lineType=cv2.LINE_8, shift=0)
            cv2.line(frame,  key_points[2],  key_points[0], (0, 255, 255), thickness=2, lineType=cv2.LINE_8, shift=0)
            if len(keypoints_)>0:
                for i in range(len(keypoints_)):
                    if i == 13 or i == 15:
                        cv2.circle(frame, (int(keypoints_[i,0]),int(keypoints_[i,1])),2, (0,0,255), 2)
                    elif i == 14 or i == 16:
                        cv2.circle(frame, (int(keypoints_[i, 0]), int(keypoints_[i, 1])), 2, (0, 255, 0), 2)
                    if i < 13:
                        cv2.circle(frame, (int(keypoints_[i, 0]), int(keypoints_[i, 1])), 2, (255, 0, 0), 2)
