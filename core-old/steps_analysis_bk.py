...
def calculate_step_length_speed(step_data, fps=60):
    ...
    step_length = abs(left_ankle_x - right_ankle_x)
    step_time = 1 / fps * (step_data.iloc[-1, :]['frame_id'] - step_data.iloc[0, :]['frame_id'])
    step_speed = step_length / step_time
...
def analysis_circle(df, stage_idx, plot=False):
    df = smooth_data_for_finding_peaks(df, ["left_ankle_x", "right_ankle_x"], sigma=2)
    ...
    while n < len(stage_idx):
        ...
        while idx < len(new_peaks):
            ...
            if idx == 0:
                ...
                step_length, step_speed, front_leg = calculate_step_length_speed(step_data)
            ...
            steps_dict["steps_info"].append(
                {
                    ...
                    "hip_max_degree": max_degree,
                    "first_step": False,
                    "steps_diff": step_length - steps_dict["steps_info"][-1]["step_length"],
                    "stride_length": step_length + steps_dict["steps_info"][-1]["step_length"]
                }
            )
            idx += 1
...
if __name__ == '__main__':
    main_analysis("/home/zhaosilei/Projects/PycharmProjects/mmlab/mmpose-1.3.1/tools/workdirs/rtmpose-m_coco25/output/73_raw.csv", smooth_sigma=15)
