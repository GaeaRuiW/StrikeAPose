...
def get_step_hip_degree_overlap(action_id: int, session: SessionDep = SessionDep):
    ...
    if not action:
        return {"data": [], "message": "Action not found"}, 404
    objects = session.query(Objects).filter(
        Objects.action_id == action_id, Objects.is_deleted == False).all()
    if not objects:
        return {"data": [], "message": "No objects found for this action"}, 404
    for obj in objects:
        x_data = []
        y_low_data = []
        y_high_data = []
        ...
        stages = session.query(Stage).filter(
            Stage.is_deleted == False, Stage.object_id == obj.id).all()
        for stage in stages:
        ...
        final_results.append({obj.name: line.dump_options_with_quotes()})
    return {"data": final_results, "message": "Success"}


@router.get("/step_hip_degree/{action_id}/{object_id}")
def get_step_hip_degree_overlap_obj(action_id: int, object_id: int, session: SessionDep = SessionDep):
    obj = session.query(Objects).filter(
        Objects.id == object_id, Objects.is_deleted == False, Objects.action_id == action_id).first()
    if not obj:
        return {"data": [], "message": "Object not found"}, 404
    x_data = []
    y_low_data = []
    y_high_data = []
    step = 1
    stages = session.query(Stage).filter(
        Stage.is_deleted == False, Stage.object_id == obj.id).all()
    if not stages:
        return {"data": [], "message": "No stages found for this object"}, 404
    for stage in stages:
        steps_info = session.query(StepsInfo).filter(
            StepsInfo.stage_id == stage.id, StepsInfo.is_deleted == False).all()
        for step_info in steps_info:
...
def get_step_width(action_id: int, session: SessionDep = SessionDep):
    ...
    for obj in objects:
        ...
        step = 1
        stages = session.query(Stage).filter(
            Stage.object_id == obj.id, Stage.is_deleted == False).all()
        for stage in stages:
            steps_info = session.query(StepsInfo).filter(
                StepsInfo.stage_id == stage.id, StepsInfo.is_deleted == False).all()
            for step_info in steps_info:
...
@router.get("/step_width/{action_id}/{object_id}")
def get_step_width_obj(action_id: int, object_id: int, session: SessionDep = SessionDep):
    obj = session.query(Objects).filter(
        Objects.id == object_id, Objects.is_deleted == False, Objects.action_id == action_id).first()
    if not obj:
    ...
    step = 1
    stages = session.query(Stage).filter(
        Stage.object_id == object_id, Stage.is_deleted == False).all()
    if not stages:
        return {"message": "No stages found for this object"}, 404
    for stage in stages:
        steps_info = session.query(StepsInfo).filter(
            StepsInfo.stage_id == stage.id, StepsInfo.is_deleted == False).all()
        for step_info in steps_info:
...
@router.get("/step_length/{action_id}/{object_id}")
def get_step_length_obj(action_id: int, object_id: int, session: SessionDep = SessionDep):
    obj = session.query(Objects).filter(
        Objects.id == object_id, Objects.is_deleted == False, Objects.action_id == action_id).first()
    if not obj:
        return {"message": "Object not found"}, 404
...
@router.get("/step_speed/{action_id}/{object_id}")
def get_speed(action_id: int, object_id: int, session: SessionDep = SessionDep):
    obj = session.query(Objects).filter(
        Objects.id == object_id, Objects.is_deleted == False, Objects.action_id == action_id).first()
    if not obj:
        return {"message": "Object not found"}, 404
...
@router.get("/step_stride/{action_id}/{object_id}")
def get_step_stride_obj(action_id: int, object_id: int, session: SessionDep = SessionDep):
    obj = session.query(Objects).filter(
        Objects.id == object_id, Objects.is_deleted == False, Objects.action_id == action_id).first()
    if not obj:
        return {"message": "Object not found"}, 404
...
@router.get("/step_difference/{action_id}/{object_id}")
def get_step_difference_obj(action_id: int, object_id: int, session: SessionDep = SessionDep):
    obj = session.query(Objects).filter(
        Objects.id == object_id, Objects.is_deleted == False, Objects.action_id == action_id).first()
    if not obj:
        return {"message": "Object not found"}, 404
...
@router.get("/support_time/{action_id}/{object_id}")
def get_support_time_obj(action_id: int, object_id: int, session: SessionDep = SessionDep):
    obj = session.query(Objects).filter(
        Objects.id == object_id, Objects.is_deleted == False, Objects.action_id == action_id).first()
    if not obj:
        return {"message": "Object not found"}, 404
...
@router.get("/liftoff_height/{action_id}/{object_id}")
def get_liftoff_height(action_id: int, object_id: int, session: SessionDep = SessionDep):
    obj = session.query(Objects).filter(
        Objects.id == object_id, Objects.is_deleted == False, Objects.action_id == action_id).first()
    if not obj:
        return {"message": "Object not found"}, 404
